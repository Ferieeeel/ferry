...
if image_requete_path is not None:
    # Convertir l'image requête en tableau numpy
    image_requete_np = np.array(bytearray(image_requete_path.read()), dtype=np.uint8)
    # Lire l'image avec OpenCV
    image = cv2.imdecode(image_requete_np, cv2.IMREAD_COLOR)  # Charger l'image en couleur (BGR)
    ...
    st.image(image_requete, caption="Image requête (Niveaux de gris)", use_column_width=True)
